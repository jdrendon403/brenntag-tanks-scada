...
const VAR_META: Record<Variable, { label: string; unit: string; color: string }> = {
  height:     { label: 'Altura',   unit: 'm',  color: '#3b82f6' },
  percentage: { label: 'Nivel',    unit: '%',  color: '#22c55e' },
  volume:     { label: 'Volumen',  unit: 'L',  color: '#8b5cf6' },
    ...
    <div className="max-w-4xl mx-auto space-y-4">
      ...
      <div className="bg-slate-800 rounded-lg p-6 border border-slate-700">
        <div className="flex justify-around flex-wrap gap-6">
          <LevelBar label="Altura"  value={height}  unit="m"  max={overflow_limit} color={alarm ? '#ef4444' : '#3b82f6'} />
          <LevelBar label="Volumen" value={volume}   unit="L"  max={volume / Math.max(percentage, 1) * 100} color="#8b5cf6" />
          <LevelBar label="Peso"    value={weight}   unit="kg" max={weight / Math.max(percentage, 1) * 100} color="#f59e0b" />
        </div>
        <div className="mt-4 grid grid-cols-2 sm:grid-cols-4 gap-3 text-center text-xs">
          <Info label="Nivel"           value={`${percentage.toFixed(1)}%`} />
          <Info label="Sobrellenado"    value={`${overflow_limit.toFixed(2)} m`} color={alarm ? 'text-red-400' : undefined} />
        ...
        {history.length === 0 ? (
          ...
          <ResponsiveContainer width="100%" height={220}>
            <LineChart data={history}>
              ...
              />
              <Line type="monotone" dataKey={variable} stroke={VAR_META[variable].color}
                dot={false} strokeWidth={2} name={VAR_META[variable].label} />
            </LineChart>
          </ResponsiveContainer>
        )}
      </div>
